...
//   	 * 2.初始化班级信息
//   	 */
////   	int updateClassByStudentBasicId(ClassIdAndStudentList lbs);
	 ...
	 * 3.查询某个班级的所有学生
	 */
   	List<StudentStatus> selectStudentByClassID(@Param("classid")String classid);
   	
   	
	/**
	 * 4.按性别与专业查询所有学生学籍信息
	 */
   	List<StudentStatus> selectStudentBySexAndMajor(StudentBasic sb);
...
//	6.根据部门、系、专业、班级参数查询所有学生
   	List<StudentStatus> getStudentStatus(DDMC ddmc);
   	
   	
//   	7.将某些学生移出某个班级
   	int updatelistclassformstudent(@Param("idlist")List<Integer> id, @Param("classId")String classId);
   ...
   //9.查询某专业未加入班级的学生
   List<StudentStatus> getStudentByMajorId(String majorId);
